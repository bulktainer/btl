...
try {
    ...
    $fileToDelete = $ctx->getWeb()->getFileByServerRelativeUrl("/sites/testfileupload/Shared Documents/attachment.txt");
    ...
    uploadFiles($localPath,$list);die;
    $fileUrl = "/sites/testfileupload/Shared Documents/CR_Slush_Invoice_Processing.docx";
    ...
    //processFiles($list ,$localPath,$settings['Url']);die;
    //downloadFile($ctx,$fileUrl,$localPath);die;
    //uploadFiles($localPath,$list);
    //Break role inheritance on the file.
    //$fileUrl = "/Shared Documents/SharePoint User Guide - 2013.docx";
    downloadFile($ctx,$fileUrl,$localPath);die;
    echo "<pre>";print_r($fileUrl);die;
    ...
    echo "<pre>";print_r($siteUsers);die;
    $ctx->load($siteUsers);
    $ctx->executeQuery();
...
function processFiles(SPList $list,$targetPath,$base)
{
    $ctx = $list->getContext();
    ...
    foreach ($files->getData() as $file) {
        print "File name: '".$base."/Shared Documents/{$file->Name}'";echo "<br>";
...
function uploadFiles($localPath, \Office365\PHP\Client\SharePoint\SPList $targetList){
    ...
    echo "<pre>";print_r(glob($searchPrefix));die;
