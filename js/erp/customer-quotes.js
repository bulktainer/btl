...
function calculateTankAdditionalInfo(){
	...
	if($('#number_of_loads').length > 0){
		...
		var tank_days = $('#tank_days').val().trim();
		...
		var no_of_tanks_required = (((parseInt(tank_days_adjustment) + parseInt(tank_days)) * parseInt(number_of_loads)) / 280).toFixed(2);
...
function update_quote_costs() {
  ...
  $('#total_tank_cost').val(converted_tank_cost);
  ...
  if($('#page').attr('data-url') == 'customer-quotes') {
	...
	if($(".approved").prop('checked') == true){
    	diff_amt = parseFloat($('#total_cost').val()) - parseFloat($('#sub_tot').val());
		margin	 = parseFloat($('#margin').val()) - parseFloat(diff_amt);
...
function perform_conversion(value, currency_from, currency_to) {
  ...
  if(currency_from != undefined && currency_to != undefined){
...
$(function() {
  if($('#page_name').val() == "short-sales-quote"){
