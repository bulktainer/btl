(function() {
	...
	var alert_success_redirection_xls = '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><i class="fa fa-exclamation-triangle"></i>Successfully exported to Excel file, Click on the links below !!!</div>';
	var alert_success_redirection_pdf = '<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><i class="fa fa-exclamation-triangle"></i>Successfully exported to PDF file, Click on the links below !!!</div>';
  	...
  	$.ajax({
        ...
        data: { 'quote_numbers': quotes , 'multi_diff_cust' : multi_diff_cust, 'customer_names' : customer_names , 'customer_ids' : customer_ids},
  	    ...
  	    success: function(data) {   
			// $("#exporting_data").val(JSON.stringify(data));			
  	      	if(data != "") 
  	      	{ 
				if(export_type == 1){  
				$.each(data, function(key, value) {			 
					...
					columns: [
						  		...
						  		{field: "Product"},
						  		{field: "Route"},
						  		{field: "TankType",title:"Tank Type"},
						  		{field: "Curr"},
						  		{field: "Rate"},
				...
				grid = $("#grid").grid({
					...
					columns: [
						  ...
						  {field: "Rate"},
						  {field: "AdditionalInfo", title:"Additional Info"},
						  {title: "Edit", field: "Edit", width: 20, type: "icon", icon: "ui-icon-pencil", tooltip: "Edit", events: { "click": Edit } },
						  {title: "Delete", field: "Delete", width: 20, type: "icon", icon: "ui-icon-trash", tooltip: "Delete", events: { "click": Delete } }
					],
				   //  pager: { enable: true, limit: 2, sizes: [2, 5, 10, 20] }
				   });
			}
	...
	function Edit(e) {
		...
		$("#fld_Customer").val(e.data.record.Customer);
		...
		$("#fld_Route").val(e.data.record.Route);
		$("#fld_TankType").val(e.data.record.TankType);
    ...
    $("#SaveExcel").click(function(){
    	...
    	if(export_type == 1){ 			
			...
			if(multi_diff_cust == 1){
				var dataToBePassed = {'content': exprt_data_string,'cid':customer_ids,'export_type': export_type,'multi_diff_cust': multi_diff_cust};
			}else{
				var dataToBePassed = {'content':exprt_data_string,'cid':customer_ids,'export_type': export_type,'multi_diff_cust': multi_diff_cust};
			}
		}else{
			...
			var dataToBePassed = {'content':exprt_data_string,'cid':customerid,'cname':customer_name};
		}
		var $this = $(this);
