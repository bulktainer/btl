...
$('#declaration_details_btn').click(function(){
	$('#declaration_details_btn i').toggleClass('fa-minus-circle fa-plus-circle');
});
$('#jobinfo_btn').click(function(){
	$('#jobinfo_btn i').toggleClass('fa-minus-circle fa-plus-circle');
});
...
$(document).on('change','#transport_mode', function(){
  ...
  $('#common-address').hide();
  $('.activity-selector,.address-selector,.supplier-selector').val('').trigger("chosen:updated"); 
  ...
  transport_mode_value = transport_mode_value.replace('deep sea shipping', 'ds_shipping');
...
$(document).on('click', '#add-supplier-cost', function(e) {
  ...
  function saveToTable(json){
	  ...
	  $.ajax({
	      type: 'POST',
	      url: appHome+'/jobtemplate-quotes/save_supp_cost',
	      //async : false,
	      data: {
	    	  'json': json,
...
function enableSafetySecurityforShip(){
	var tmp_quote  = $('#customer_quote_id').val();
	var tmp_randamnumber = $('#random_number').val();
	...
	$.ajax({
    	...
    	data: {
           	'tmp_quote'  : tmp_quote,
            'tmp_randamnumber'    : tmp_randamnumber,
            'action_type' : 'checkShipExportImport'
    	},
   		success: function(response){
			var response = JSON.parse(response);
			$('#q_safety_security_enabled').val(response.ss_enabled_ship);
			
			if($('#declaring').val() != "declare"){
				
				if(response.ss_enabled_ship > 0){
					$('#safety-details').show();
				} else {
					$('#safety-details').hide();
				}
...
function getsupplierlist(applyrate){
    ...
    $.ajax({
        type: 'POST',
        url: appHome+'/jobtemplate-quotes/supplier_rate_list',
        data: {
...
$(document).on('click', '.radio-transport-mode-route', function(e) {
	...
	if(transport_mode == 2 && $('#tank_state_2').val() == 'loaded'){
		...
		activitySave = 'RAILE';
	}else if(transport_mode == 3 && $('#tank_state_3').val() == 'loaded'){ 
		activitySave = 'SHIP';
...
function uploadFile() {
	...
	xhr.open("POST", appHome + '/jobtemplate-quotes/job-template-file-upload');
	xhr.send(fd);
}
...
$(document).on('change', '#collection,#delivery', function(e) {
	var collection = $("#collection").val();
	var delivery   = $("#delivery").val();

	if(collection != '' && delivery != ''){
		checkExportImport(collection,delivery);
	}
...
function checkExportImport(collection,delivery){
	...
	$.ajax({
    	...
    	data: {
            ...
            'delivery'    : delivery,
            'action_type' : 'checkExportImport'
    	},
   		success: function(response){
				if(response =='EXPORT' || response == 'IMPORT'){
            		$('#declaration-details').show();
					$('#safety-details').show();
            		$('#declaring').val('declare');
            	}else{
            		$('#declaration-details').hide();
					q_safety_security_enabled = parseInt($('#q_safety_security_enabled').val()); 
					if(q_safety_security_enabled <= 0){
						$('#safety-details').hide();
					}
            		$('#declaring').val('');
            	}
            	...
            	if(response == 'IMPORT'){
            		$("#import_export").val('IMPORT');
            		$("#customs_to").text('GB Customs');
            		$("#customs_from").text('EU Customs');

            	}else if (response == 'EXPORT') {
            		$("#import_export").val('EXPORT');
            		$("#customs_from").text('GB Customs');
            		$("#customs_to").text('EU Customs');
            	}
        },
        error: function(response){
            $('html, body').animate({ scrollTop: 0 }, 400);
...
function getCustomerData(customer){
	...
	$.ajax({
    	...
    	data: {
    		'customer'    :  customer,
            'action_type' : 'getCustomerData'
    	},
   		success: function(response){
             $("#company_name_consignor").val(response.cust_name);
             $("#consignor_adress1").val(response.cust_addr1);
             $("#consignor_adress2").val(response.cust_addr2);
             $("#consignor_town").val(response.cust_town);
             $("#consignor_state").val(response.cust_state);
             $("#consignor_postcode").val(response.cust_postcode);
             $("#eori_consignor").val(response.cust_eori);
             $("#consignor_country").val(response.cust_country);
		},
        error: function(response){
            $('html, body').animate({ scrollTop: 0 }, 400);
            $('form').find('#response').empty().prepend(alert_error).fadeIn();
...
$(function() {
	if($('#page_name').val() == "short-sea-job-temp"){
