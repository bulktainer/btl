...
*/
$('.edit-vgm-route,.create-vgm-route').click(function(e){
  $('.highlight').removeClass('highlight');
  e.preventDefault();
  var form = '#'+$(this).closest('form').attr('id'),
      success = [],
      vgm_id = $('input[name="vgm_id"]').val(),
  ...
  function isrouteExist(fromTown,toTown,supplier,button) {
	...
	if(button.hasClass('edit-vgm-route')){
  		var type = "update";
  	}
  	if(button.hasClass('create-vgm-route')){
  		var type = "create";
  	}
	var fromTownVal = fromTown.val();
	var toTownVal = toTown.val();
	var supplierVal = supplier.val();
	var vgmID = $('#vgm_id').val();
  ...
  * update edit-vgm-route
  */
  if($(this).hasClass('edit-vgm-route')){
   ...
   * create-vgm-route
   */
   if($(this).hasClass('create-vgm-route')){
     if(check_fields === true){
       $('html, body').animate({ scrollTop: 0 }, 400);
       $('form').find('#response').empty().prepend(alert_required).fadeIn();
     } else {
